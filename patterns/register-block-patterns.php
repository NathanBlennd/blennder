 ...
 */
function blennder_search_title() {
	if ( isset( $_GET['s'] ) ) {
		$search_term = sanitize_text_field( wp_unslash( $_GET['s'] ) );
		/* translators: %s: Search term. */
		return isset( $search_term ) ? sprintf( esc_html__( 'Search results for "%s"', 'blennder' ), esc_html( $search_term ) ) : __( 'Search results', 'blennder' );
	}
}
 ...
 * @return void
 */
function blennder_register_block_patterns() {
	...
	register_block_pattern_category(
		'images',
		array( 'label' => esc_html__( 'Images', 'blennder' ) )
	);
	...
	foreach ( $block_patterns as $block_pattern ) {
		...
		register_block_pattern(
			'blennder/' . $block_pattern,
			require $pattern_file
		);
	}
}
add_action( 'init', 'blennder_register_block_patterns', 9 );
 ...
 * @return void
 */
function blennder_unregister_patterns() {
	unregister_block_pattern( 'core/query-small-posts' );
	unregister_block_pattern( 'core/query-large-title-posts' );
	unregister_block_pattern( 'core/query-offset-posts' );
	unregister_block_pattern_category( 'header' );
}
add_action( 'init', 'blennder_unregister_patterns', 10 );
